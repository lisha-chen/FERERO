...
def ferero_search(multi_obj_fg, pref_vec, x=None,
                  max_iters=200, n_dim=20, step_size=1, A = np.eye(2)):
    """
    FERERO
    """
    ...
    for t in range(max_iters):
        ...
        weights, nd = get_d_ferero(grads, F, pref_vec, A)
...
def get_d_ferero(grads, F, pref_vec, A = np.eye(2)):
    """
...
def get_d_ferero_single(grads, F, grad_lamt, 
            init_lam_f, init_lam_h, 
            pref_vec, iter_K, A = np.eye(2)):
    """
    calculate the gradient direction for FERERO
    """
    nobj, dim = grads.shape
